(function () {
  ...
  // só faz isso quando o TOC está realmente visível (aberto ou pinado)
  if (!tocEl.classList.contains("is-open") && !tocEl.classList.contains("is-pinned")) return;

  const pad = 14; // respiro visual
  const cTop = tocEl.scrollTop;
  const cBottom = cTop + tocEl.clientHeight;

  // posição do link dentro do container (não da janela)
  const elTop = linkEl.offsetTop;
  const elBottom = elTop + linkEl.offsetHeight;

  // se ficou acima, sobe
  if (elTop - pad < cTop) {
    tocEl.scrollTo({ top: Math.max(0, elTop - pad), behavior: "smooth" });
    return;
  }

  // se ficou abaixo, desce
  if (elBottom + pad > cBottom) {
    tocEl.scrollTo({ top: elBottom - tocEl.clientHeight + pad, behavior: "smooth" });
  }
}
    ...
    if ("IntersectionObserver" in window && map.size) {
      ...
      const io = new IntersectionObserver((entries) => {
        ...
        // ✅ faz o TOC acompanhar a rolagem do conteúdo
        keepActiveInView(tocEl, a);
...
(function syncHeaderHeightVar(){
  ...
  // quando a tela muda (mobile/desktop) e quando fontes carregam
  window.addEventListener("resize", setHeaderH);
  window.addEventListener("orientationchange", setHeaderH);

  // garante após render/layout
  requestAnimationFrame(setHeaderH);
  setTimeout(setHeaderH, 150);
})();
